...
perform_install() {
  ...
  if [[ -n "$_is_frash_install" ]]; then
    ...
    echo
    echo -e "$(tbold)AGN-UDP has been successfully update to $VERSION from Hysteria 1.$(treset)"
    echo
    echo -e "$(tred)AGN-UDP 2 uses a completely redesigned protocol & config, which is NOT compatible with the version 1.x.x in any way.$(treset)"
    echo
    echo -e "\t+ Take a look at the behavior changes in AGN-UDP 2 at $(tblue)https://hysteria.network/docs/misc/2-vs-1/$(treset)"
    ...
    echo -e "\t+ Migrate server config file to the AGN-UDP 2 at $(tred)$CONFIG_DIR/config.yaml$(treset)"
